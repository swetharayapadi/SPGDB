...
$strain = $cgi -> param('organism');
$db = "SPDB";
$host = "localhost";
$user = "root";
$password = "";
$dbh = DBI -> connect("DBI:mysql:$db:$host",$user,$password);
$query = "select * from protein_list where Entry = '$protein' and Organism = '$strain'";
...
if (defined $entry) {
	...
	print "<p class = \"MCContent2\">The protein entry <b> \"$protein\"</b> is not found in <i>$strain</i></p>";
}
print "</div>";
print "</body>";
print "</html>";
