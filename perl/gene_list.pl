...
$sql = "select count(*) from gene_list where Organism = '$strain'";
...
if ($totalcount != 0){
print "<p class = \"MCHead\">List of genes in <i> $strain </i></p>";
...
print "<center><form method = \"GET\" action = \"category_list.pl?pagenum=1&strain=$strain&category=$category\" id = \"category_form\">";
print "<input type = \"hidden\" value = \"$strain\" id = \"strain\" name = \"strain\">";
print "<input type = \"radio\" value = \"CDS\" id = \"CDS\" name = \"category\" onclick = \"category_Wise()\"><j style = \"font: 12px verdana;\"> CDS </j>";
print "<input type = \"radio\" value = \"tRNA\" id = \"tRNA\" name = \"category\" onclick = \"category_Wise()\"><j style = \"font: 12px verdana;\"> tRNA </j>";
print "<input type = \"radio\" value = \"rRNA\" id = \"rRNA\" name = \"category\" onclick = \"category_Wise()\"><j style = \"font: 12px verdana;\"> rRNA </j>";
print "<input type = \"radio\" value = \"ncRNA\" id = \"ncRNA\" name = \"category\" onclick = \"category_Wise()\"><j style = \"font: 12px verdana;\"> ncRNA </j>";
print "<input type = \"radio\" value = \"tmRNA\" id = \"tmRNA\" name = \"category\" onclick = \"category_Wise()\"><j style = \"font: 12px verdana;\"> tmRNA </j>";
print "</form></center>";
print "<table class = \"pageGI\"><tr>";
print "<td>&nbsp&nbsp</td><td width=35><a style='text-decoration: none;' href = ?pagenum=".$page_info->first_page."&strain="."$strnn".">&nbsp&nbsp<img class = \"pagefirst\" src = \"/SPGDB/images/first.png\"/> </a></td>";
print "<td width=35><a style='text-decoration: none;' href = ?pagenum=".$page_info->previous_page."&strain="."$strnn"."> <img class = \"page1\" src = \"/SPGDB/images/backward.png\"/></a></td>";
# Print the page numbers of the current set
foreach my $page (@{$page_info->pages_in_set()}) {
	if($page == $page_info->current_page()) {
  		print "<td width=35><a style = 'text-decoration: none;font: 12px verdana;color: white; background-color: black; padding: 5px; border-radius: 3px;' href = ?pagenum=".$page."&strain="."$strnn"."><b>$page</b></a></td> ";
		} else {
  		print "<td width=35><a style='text-decoration: none;font: 12px verdana; color: black; background-color: rgba(192,192,192,0.80); padding: 5px; border-radius: 3px;' href = ?pagenum=".$page."&strain="."$strnn".">$page</a></td> ";
	}
}
print "<td width=35><a style='text-decoration: none;' href = ?pagenum=".$page_info->next_page."&strain="."$strnn"."> <img class = \"page1\" src = \"/SPGDB/images/forward.png\"/></a></td>";
print "<td width=35><a style='text-decoration: none;' href = ?pagenum=".$page_info->last_page."&strain="."$strnn"."> <img class = \"pagelast\" src = \"/SPGDB/images/last.png\"/></a></td></tr></table>";
...
$sql1 = "select * from gene_list where Organism = '$strain' limit $f,$g";
...
print "</div>";
	...
	print "<p class = \"MCHead\" style= \"margin-left: 10px;\">No data available for <i>$strain</i></p>";
