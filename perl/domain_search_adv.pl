...
$sql = "select count(*) from protein_list where Organism = '$strain' and domains LIKE '\%$domain\%'";
...
print "<body>";
if ($totalcount != 0){
print "<p class = \"MCHead\">List of proteins with <b> $domain </b> in <i> $strain </i></p>";
#print "####### $strain ####";
print "<div>";  
...
print "<td>&nbsp&nbsp</td><td width=35><a style='text-decoration: none;' href = \"javascript:go_Domain('$first','$domain','$strnn');\"><img class = \"pagefirst\" src = \"/SPGDB/images/first.png\"/> </a></td>";
print "<td width=35><a style='text-decoration: none;' href = \"javascript:go_Domain('$previous','$domain','$strnn');\"> <img class = \"page1\" src = \"/SPGDB/images/backward.png\"/></a></td>";
# Print the page numbers of the current set
foreach my $page (@{$page_info->pages_in_set()}) {
	if($page == $page_info->current_page()) {
  		print "<td width=35><a style = 'text-decoration: none;font: 12px verdana;color: white; background-color: black; padding: 5px; border-radius: 3px;' href = \"javascript:go_Domain('$page','$domain','$strnn');\"><b>$page</b></a></td> ";
		} else {
  		print "<td width=35><a style='text-decoration: none;font: 12px verdana; color: black; background-color: rgba(192,192,192,0.80); padding: 5px; border-radius: 3px;' href = \"javascript:go_Domain('$page','$domain','$strnn');\">$page</a></td> ";
	}
}
print "<td width=35><a style='text-decoration: none;' href = \"javascript:go_Domain('$next','$domain','$strnn');\"> <img class = \"page1\" src = \"/SPGDB/images/forward.png\"/></a></td>";
print "<td width=35><a style='text-decoration: none;' href = \"javascript:go_Domain('$last','$domain','$strnn');\"> <img class = \"pagelast\" src = \"/SPGDB/images/last.png\"/></a></td></tr></table>";
...
$sql1 = "select * from protein_list where Organism = '$strain' and domains LIKE '\%$domain\%' limit $f,$g";
...
print "</div>";
	...
	print "<p class = \"MCHead\" style= \"margin-left: 10px;\"> No proteins with <b>$domain</b> available for <i>$strain</i></p>";
}
