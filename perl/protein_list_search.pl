...
$sql = "select count(*) from protein_list where Organism = '$strain'";
...
if ($totalcount != 0){
print "<p class = \"MCHead\">List of proteins in <i> $strain </i></p>";
...
$sql1 = "select * from protein_list where Organism = '$strain' limit $f,$g";
...
while (@list = $query->fetchrow_array){
	...
	print "<a style = \" text-decoration: none; background-color: rgba(192,192,192,0.80); color: black; padding: 3px; border-radius: 5px; \"href = \"proteinsearch_list.pl?protein=$entry\"> Show more </a></td></tr>";
}
print "</table></br>";
print "</div>";
}
	...
	print "<a href = \"strain_list.pl?pagenum=1\"><img class = \"backGI2\" src = \"/SPGDB/images/back-button-png.svg\"/></a>";
}
