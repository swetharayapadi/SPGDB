...
$sql = "select count(*) from protein_list where Organism = '$strain' and Subcellularlocation LIKE ('\%$location\%')";
...
print "<td>&nbsp&nbsp</td><td width=35><a style='text-decoration: none;' href = \"javascript:go_locate('$first','$location','$strnn');\"><img class = \"pagefirst\" src = \"/SPGDB/images/first.png\"/> </a></td>";
print "<td width=35><a style='text-decoration: none;' href = \"javascript:go_locate('$previous','$location','$strnn');\"> <img class = \"page1\" src = \"/SPGDB/images/backward.png\"/></a></td>";
# Print the page numbers of the current set
foreach my $page (@{$page_info->pages_in_set()}) {
	if($page == $page_info->current_page()) {
  		print "<td width=35><a style = 'text-decoration: none;font: 12px verdana;color: white; background-color: black; padding: 5px; border-radius: 3px;' href = \"javascript:go_locate('$page','$location','$strnn');\"><b>$page</b></a></td> ";
		} else {
  		print "<td width=35><a style='text-decoration: none;font: 12px verdana; color: black; background-color: rgba(192,192,192,0.80); padding: 5px; border-radius: 3px;' href = \"javascript:go_locate('$page','$location','$strnn');\">$page</a></td> ";
	}
}
print "<td width=35><a style='text-decoration: none;' href = \"javascript:go_locate('$next','$location','$strnn');\"> <img class = \"page1\" src = \"/SPGDB/images/forward.png\"/></a></td>";
print "<td width=35><a style='text-decoration: none;' href = \"javascript:go_locate('$last','$location','$strnn');\"> <img class = \"pagelast\" src = \"/SPGDB/images/last.png\"/></a></td></tr></table>";
...
$sql1 = "select * from protein_list where Organism = '$strain' and Subcellularlocation LIKE ('\%$location%') limit $f,$g";
...
print "</div>";
	...
	print "<p class = \"MCHead\" style= \"margin-left: 10px;\"> No proteins in $location available for <i>$strain</i></p>";
